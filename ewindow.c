 ...
 * completely the same; most of the work has to do with reframing the window,
 * and picking a new dot. We share the code by having "move down" just be an
 * interface to "move up". Magic. Bound to "C-X C-N".
 */
 ...
 * (this command does not really move "."; it moves the frame). Bound to "C-X
 * C-P".
 */
 ...
 * to "C-X 1". Try to set the framing so that "." does not have to move on
 * the display. Some care has to be taken to keep the values of dot and mark
 * in the buffer structures right if the distruction of a window makes a
...
/*
 * Delete the current window, placing its space in the window above, or, if
 * it is the top window, the window below. Bound to C-X 0.
 */
 ...
 * (Two line windows can be split when mode lines are disabled) An argument
 * of 1 forces the cursor into the upper window, an argument of two forces
 * the cursor to the lower window.  The only other error that is possible is
 * a "malloc" failure allocating the structure for the new window.  Bound to
 * "C-X 2".
 * 
 */
...
int newsize (int f, int n)	  /* resize the screen, re-writing the screen */
{
  ...
  int lastline;			  /* screen line of last line of current
				   * window */
  int cmark;			  /* current mark */
...
int new_col_org (int f, int n)	  /* reposition the screen, re-writing the
				   * screen */
{
  register WINDOW *wp;
...
int new_row_org (int f, int n)	  /* reposition the screen, re-writing the
				   * screen */
{
  register WINDOW *wp;
